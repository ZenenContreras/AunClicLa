...
const ProductsPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-2 sm:px-4">
      <h1 className="text-2xl sm:text-3xl font-bold mb-6">{t('catalog.products')}</h1>
      {/* Tabs de subcategorías */}
      <motion.div layout className="flex gap-2 mb-8 overflow-x-auto" role="tablist">
        <motion.button
          layout
          aria-label={t('catalog.all')}
          className={`px-3 py-1.5 rounded-full font-semibold transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 text-sm sm:text-base ${
            selectedSub === null
      ...
      {hasMore && !loading && (
        <div className="flex justify-center mt-8">
          <button
            ...
            className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-all shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
